...
<%-- 							<input type='hidden' name='boardWriterName' value='${ b.boardWriterName }'> --%>
							...
							<div class="text-center">
								...
								<c:if test="${ b.cateNo == 101  || b.cateNo == 103 }">
		...
		<c:if test="${ b.cateNo == 101 || b.cateNo == 103 }">
			<div class="container text-center">
				<div class="row">
					...
					<div class="col-8 content-center" style='background: white; border-radius: 25px;'>
					<div class="col-8 content-center" style='background: white; border-radius: 25px;'>
						<h3 style="float: left; border-left: 5px solid #65B741; padding-left: 10px; display: inline-block; margin-bottom: 15px;" > 운영자 답변</h3>
					</div>
						<table class="table">
							<thead>
								<tr>
							...
							<tbody class='table-group-divider'>
								<c:if test="${ !empty quReply }">
									<c:forEach items="${ quReply }" var='r'>
										<tr>
											<td>${ r.reContent }</td>
											<td>${ r.modifyDate }</td>
											<td>
												<input type='hidden' value='${ r.reNo }' name='reNo'>
												<c:if test="${ loginUser.memGrade == 'WORKER' || loginUser.memGrade == 'MANAGER' }">
	...
	<script>
		...
		if(document.getElementById('insertReply') != null){
			document.getElementById('insertReply').addEventListener("click", () =>{
				$.ajax({
					url:"${contextPath}/insertQuBoReply.admin",
					...
					success: data => {
						...
						for(const reply of data){
							...
							if('${loginUser.memGrade} == WORKER' || '${loginUser.memGrade} == MANAGER'){
		...
		tbody.addEventListener('click', e => {
			...
			if(targetButton != null){
				...
				// content가 담긴 Td
				const contentTd = clickTd.parentElement.children[0];
				if(targetButton.children[0].id == 'updateReply'){
					...
					// 수정 버튼 > 완료 버튼 childred[0]은 reNo값
				...
				// 댓글 삭제	
					...
					if(confirm("정말 삭제하시겠습니까?")){
						$.ajax({
							url:'${contextPath}/deleteQuBoReply.admin',
							data:{reId:reId},
							success: data => {
								if(data == 'success'){
					...
					$.ajax({
						url:"${contextPath}/updateQuBoReply.admin",
		...
		const buttonChange = (type) => {
			...
			console.log(beforeContentInput)
			// 내용 Td
			const contentTd = beforeTd.parentElement.children[0];
			//버튼 변경
